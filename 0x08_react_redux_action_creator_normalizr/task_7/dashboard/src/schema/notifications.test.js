...
describe('Notifications data', () => {
	it('Read some data from a json', () => {
		const data = [
			...
			notificationId = getAllNotificationsByUser(
				'5debd764a7c57c7839d722e9'
			);

		expect(notificationId).toEqual(expect.arrayContaining(data));
	});
});
...
describe('Normalized data', () => {
	it('normalized result', () => {
		...
		expect(normalizedData.result).toEqual(expect.arrayContaining(data));
	});
