...
describe('With CourseList Empty', () => {
	...
	it('Third row', () => {
		expect(wrapper.find('CourseListRow').at(2).html()).equal(
			'<tr><td><input type="checkbox"/>No course available yet</td><td></td></tr>'
		);
	});
});
...
describe('With CourseList containing elements', () => {
	...
	it('Third row', () => {
		expect(wrapper.find('CourseListRow').at(2).html()).equal(
			'<tr><td><input type="checkbox"/>ES6</td><td>60</td></tr>'
		);
	});

	it('Fourth row', () => {
		expect(wrapper.find('CourseListRow').at(3).html()).equal(
			'<tr><td><input type="checkbox"/>Webpack</td><td>20</td></tr>'
		);
	});

	it('Fifth row', () => {
		expect(wrapper.find('CourseListRow').at(4).html()).equal(
			'<tr><td><input type="checkbox"/>React</td><td>40</td></tr>'
		);
	});
});
